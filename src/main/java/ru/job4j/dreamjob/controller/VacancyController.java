...
import ru.job4j.dreamjob.services.SimpleVacancyService;
import ru.job4j.dreamjob.services.VacancyService;
...
public class VacancyController {
    ...
    public String delete(Model model, @PathVariable int id) {
        var isDeleted = vacancyService.findById(id).isEmpty();
        if (isDeleted) {
            model.addAttribute("message", "Вакансия с указанным идентификатором не найдена");
            return "errors/404";
        }
